//sophia walton, srw9rx sum_benchmarks.c
#include <stdlib.h>
...
unsigned short sum_AVX(long size, unsigned short *a) {
    __m256i partial_sums = _mm256_setzero_si256(); //declare the partial sums as a vector
    for (int i = 0; i < size; i += 16) {
        __m256i sums_16 = _mm256_loadu_si256((__m256i*) &a[i]); //load the array into a vector value
        partial_sums = _mm256_add_epi16(partial_sums, sums_16); //add the values from array into the sum vector
    }
    unsigned short extracted_partial_sums[16];
    _mm256_storeu_si256((__m256i*) &extracted_partial_sums, partial_sums); // extract partial sums 
    //return all items added together
...
function_info functions[] = {
    ...
    {sum_AVX, "sum with AVX instructions by 16"},
    // add entries here!
    {NULL, NULL},
};
